...
uint256 hashGenesisBlock("0xce33e7281dd8e98deea3cc15c8019d2aae6e4d47308a385f086d2254cefb0f15");
static CBigNum bnProofOfWorkLimit(~uint256(0) >> 20); // Insanitycoin: starting difficulty is 1 / 2^12
...
const string strMessageMagic = "Insanitycoin Signed Message:\n";
...
{
    // Insanitycoin: IsDust() detection disabled, allows any valid dust to be relayed.
...
int64 static GetBlockValue(int nHeight, int64 nFees)
{
    ...
    if(nHeight > 36037 && nHeight < 121718)
	{
	   nSubsidy = 0.48 * COIN;
	} else if(nHeight > 121717 && nHeight < 252758)
	{
	   nSubsidy = 0.24 * COIN;
	} else if(nHeight > 252757)
	{
	   nSubsidy = 0.12 * COIN;
	}
    return nSubsidy;
...
static const int64 nTargetTimespan = 12 * 60 * 60; // Insanitycoin: 12 hours
static const int64 nTargetSpacing = 3 * 60; // Insanitycoin: 2 minutes
...
unsigned int static GetNextWorkRequired(const CBlockIndex* pindexLast, const CBlockHeader *pblock)
{
    ...
    // Insanitycoin: This fixes an issue where a 51% attack can change difficulty at will.
...
bool CBlock::CheckBlock(CValidationState &state, bool fCheckPOW, bool fCheckMerkleRoot) const
{
    ...
    // Insanitycoin: Special short-term limits to avoid 10,000 BDB lock limit:
...
{
    // Insanitycoin: temporarily disable v2 block lockin until we are ready for v2 transition
    return false;
...
bool LoadBlockIndex()
{
    if (fTestNet)
    {
        ...
        hashGenesisBlock = uint256("0xce33e7281dd8e98deea3cc15c8019d2aae6e4d47308a385f086d2254cefb0f15");
    }
...
bool InitBlockIndex() {
    ...
    if (!fReindex) {
        ...
        // Genesis block
        const char* pszTimestamp = "Reuters 12/Feb/2014 Cyber attack on bitcoin a big warning to currency's users";
        CTransaction txNew;
        txNew.vin.resize(1);
        txNew.vout.resize(1);
        txNew.vin[0].scriptSig = CScript() << 486604799 << CBigNum(4) << vector<unsigned char>((const unsigned char*)pszTimestamp, (const unsigned char*)pszTimestamp + strlen(pszTimestamp));
        txNew.vout[0].nValue = 1 * COIN;
        txNew.vout[0].scriptPubKey = CScript() << ParseHex("04708bf48b2bca1e8c251df85a28ac781dba16b608cea6a25f12ad3b5de2675dba6a3741cb2d108efa569d8d11002fc42c7954fccd43ea734841d06d1c5cbb64b9") << OP_CHECKSIG;
        CBlock block;
        block.vtx.push_back(txNew);
        block.hashPrevBlock = 0;
        block.hashMerkleRoot = block.BuildMerkleTree();
        block.nVersion = 1;
        block.nTime    = 1392256514;
        block.nBits    = 0x1e0ffff0;
        block.nNonce   = 385646943;
        ...
        assert(block.hashMerkleRoot == uint256("0x01d42cda807e12eacbfc223e13d1f486e5f8886e385a1d645f00b4d873278ffd"));
...
unsigned char pchMessageStart[4] = { 0xfb, 0xc0, 0xb6, 0xdb }; // Insanitycoin: increase each by adding 2 to bitcoin's value.
...
bool CheckWork(CBlock* pblock, CWallet& wallet, CReserveKey& reservekey)
{
    ...
    // Found a solution
    {
        ...
        if (pblock->hashPrevBlock != hashBestChain)
            return error("InsanitycoinMiner : generated block is stale");
        ...
        if (!ProcessBlock(state, NULL, pblock))
            return error("InsanitycoinMiner : ProcessBlock, block not accepted");
    }

    return true;
}

void static InsanitycoinMiner(CWallet *pwallet)
{
    printf("InsanitycoinMiner started\n");
    SetThreadPriority(THREAD_PRIORITY_LOWEST);
    RenameThread("insanitycoin-miner");
    ...
    try { loop {
        ...
        printf("Running InsanitycoinMiner with %"PRIszu" transactions in block (%u bytes)\n", pblock->vtx.size(),
    ...
    {
        printf("InsanitycoinMiner terminated\n");
        throw;
    }
}
...
void GenerateBitcoins(bool fGenerate, CWallet* pwallet)
{
    ...
    for (int i = 0; i < nThreads; i++)
        minerThreads->create_thread(boost::bind(&InsanitycoinMiner, pwallet));
}
